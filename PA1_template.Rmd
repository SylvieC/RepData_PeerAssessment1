...
First I create two datasets that are a subset of the main dataset, 
```{r echo=TRUE}
 weekends <- filter(complete_df, day_type == "weekend")
 weekdays <- filter(complete_df, day_type == "weekday")

by_interval_weekdays <- aggregate(weekdays$steps, list(interval = weekdays$interval), mean)
by_interval_weekdays<- mutate(by_interval_weekdays, steps_average = round(x))
by_interval_weekdays <- select(by_interval_weekdays, interval, steps_average)

by_interval_weekends <- aggregate(weekends$steps, list(interval = weekends$interval), mean)
by_interval_weekends<- mutate(by_interval_weekends, steps_average = round(x))
by_interval_weekends <- select(by_interval_weekends, interval, steps_average)

head(by_interval_weekdays)
head(by_interval_weekends)
```
...
```{r echo=TRUE}
par(mfrow=c(2,2))
plot(by_interval_weekends$interval, by_interval_weekends$steps_average, type="l", xlab= "Interval Number", ylab= "Average of Steps taken",ylim=c(0,200), col="blue" , lwd=2, main=" Weekends")
plot(by_interval_weekdays$interval, by_interval_weekdays$steps_average, type="l", xlab= "Interval Number", ylab= "Average of Steps taken",ylim=c(0,200), col="blue" , lwd=2, main=" Weekdays")
```
 
 We see a difference in the average of the steps in weekdays and weekends. 
